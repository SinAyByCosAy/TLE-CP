...
public class SubtreeQueries {
    ...
    public static void main(String[] args){
        ...
        List<Integer>[] adj = new ArrayList[n + 1]; //map of nodes
        ...
        dfs(1, 0, adj, inTime, outTime); //O(N + M)
        int len = 2 * n;
        int[] euler = new int[len]; //euler tour array of values
        for(int i = 1; i <= n; i++){ //setting a node's value at it's in time idx and out time idx
            euler[inTime[i]] = arr[i];
            euler[outTime[i]] = arr[i];
        }
        ...
        for(int i = 1; i <= q; i++){
            ...
            if(type == 1){
                ...
                update(1, 0, len - 1, outTime[x], val, sgt);
            }else{
                int x = sc.nextInt();
                System.out.println((query(1, 0, len - 1, inTime[x], outTime[x], sgt)) / 2);
            }
        }
    }
    ...
    public static void update(int idx, int start, int end, int target, int val, long[] sgt){
        if(start == end){//reached the exact node, leaf
            sgt[idx] = val;
            return;
        }
        int mid = (start + end) / 2;
