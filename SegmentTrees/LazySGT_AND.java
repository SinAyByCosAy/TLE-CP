...
public class LazySGT_AND {
    ...
    public static void update(int idx, int start, int end, int l, int r, int x, int[] sgt, int[] lsgt){
        ...
        if(start >= l && end <= r){
            sgt[idx] = x; //AND of all elements as X, will be X itself
            lsgt[idx] = x;
            return;
        }
        int mid = (start + end) / 2;
        //default value, can't be an answer
        if(lsgt[idx] != -1) updateChildren(idx, sgt, lsgt);
    ...
    public static int query(int idx, int start, int end, int l, int r, int[] sgt, int[] lsgt){
        if(end < l || start > r) return -1; //all 32 set bits, x & (-1) = x
